package me.cg360.mod.warptocrosshair.config.helper;
...
public class DefaultValueTracker {
    ...
    public void saveDefaults() {
        if(this.defaultValues != null) {
            WarpToCrosshairMod.getLogger().warn("Tried to re-save the defaults variables for object. These are locked!");
            return;
        }
        ...
        for(Field field: this.getClass().getDeclaredFields()) {
            try {
                ...
                WarpToCrosshairMod.getLogger().warn("Unable to get value when saving defaults! Unexpected! [%s]".formatted(err.getMessage()));
                return;
            }
        }
    }
